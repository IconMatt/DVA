...
- `--color-ironbark-50` (#817f7d) fails WCAG AA as **text** on white — 4.03:1 against the 4.5:1 minimum. It shipped to staging and failed the 12 Jun 2026 Lighthouse audits (service-search floating labels, search-overlay submit). Muted text reads through `--color-ink-muted`, which must resolve to `--color-ironbark-75` (#66615b, 6.1:1) or darker; keep the 50 tint for non-text uses only (hairlines, icon tints). The failure is symmetric — white text ON a 50-tint background fails the same way.
- `npm run dev` (tailwind `--watch`) writes **unminified** `css/main.css`. Committing while the watcher runs captures the ~6,100-line dev artifact instead of the minified build — this happened and needed a correction commit. Always run `npm run build` before a commit that includes `css/main.css`. Side effect: the dirty build file can make `git checkout` refuse to switch branches with a one-word "Aborting" — check `git status` for `css/main.css` before assuming a checkout worked.
- Running `npm run build` before committing is necessary but **not sufficient while the watcher is live**: `tailwind --watch` re-clobbers `css/main.css` with the unminified dev build in the gap between `npm run build` and `git add`, so a commit issued as a separate step still captured the ~6,100-line artifact (happened, needed a correction commit). Before committing `css/main.css`, either stop the watch/dev-server process first, or build + `git add` + commit atomically in one command so the watcher has no window. Verify with `git show HEAD:css/main.css | wc -l` — it must be 1.
