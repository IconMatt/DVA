...
This is a Mercury-style structure: the Twig template lives in `components/<name>/<name>.twig` alongside a `<name>.component.yml` schema. **CVA is not used** in this project — BEM classes are applied directly. Conditional classes are computed in preprocess.
...
2. **Per-component CSS** (Mercury-style): each SDC folder owns a `<name>.css` (or `<name>.tailwind.css`) that's `@import`ed into the theme's `src/main.css`. Same final output; finer-grained authoring.
